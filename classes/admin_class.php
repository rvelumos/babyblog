...
class Admin{
  ...
  public function adminLog($message){
		...
		if($page == "")
			$page = "admin";
		$date=time();
		$sql="INSERT INTO ".$prefix."log (ip, date, page, user, url, browser, webhost, os_server, os_user, referrer, message, protocol, method) VALUES('".$ip."', NOW(), '$page', '".$_SESSION['admin_name']."', '".$url."','".$browser."','".$webhost."','".$server."','".$server_user."','".$referer."','".$message."', '".$protocol.
"','".$method."')";
	...
	public function getPolls(){
        ...
        $this->adminLog("Pagina polls geopend");
	...
	public function addPoll(){
		...
		if(isset($_POST['add_poll'])){
            ...
            $this->adminLog("poll $question toegevoegd");
          ...
          if(!$result = $this->link->query($sql))
             ...
             $this->adminLog("Poll {$_SESSION['question']} geopend");
              ...
              foreach($answer as $key => $value){
                ...
                if($value != ""){                  
                  ...
                  if(!$result = $this->link->query($sql))
                    ...
                    $answer = TRUE;
                    $this->adminLog("Antwoord $value toegevoegd bij poll_id $poll_id");
                  }
                }else{
	...
	public function editPoll($id){
      ...
      $this->adminLog("Poll $id opgevraagd");
		...
		if($result->num_rows > 0){
			$question = $result->fetch_array(MYSQLI_BOTH);
                ...
                $this->adminLog("Vraag $id aangepast naar $new_question");
            ...
            $this->adminLog("Antwoord $answer opgevraagd");
					if($result->num_rows > 0){
              ...
              $this->adminLog("Antwoord $answer verwijderd");
							die();
						}
					}else{
            ...
            $value = trim(mysqli_real_escape_string($this->link, $value));
                  ...
                  $this->adminLog("Nieuw antwoord toegevoegd bij poll $id");
                ...
                //update an old answer
                  ...
                  $this->adminLog("Antwoord $answer_id van poll $id aangepast");
								$msg['answer'] = "<i>Antwoord(en) succesvol opgeslagen</i>";
							}
						}else{
              ...
              $this->adminLog("Actieve pollvraag opgevraagd");
                ...
                $this->adminLog("Status van poll $id aangepast naar {$question['status']}");
						}else{
          ...
          $this->adminLog("Antwoorden van poll $id opgevraagd");
    ...
    //delete related answers first
		if($result->num_rows > 0){
          ...
          $this->adminLog("Poll $id met gerelateerde vragen verwijderd");
			}
	...
	public function pollStats($id){
		global $prefix, $settings;
          ...
          $this->adminLog("Aantal antwoorden opgeteld van $poll_id");
          ...
          $this->adminLog("Antwoorden van poll $id opgevraagd voor statieken");
	...
	public function addCt($pre, $cnt_table, $short, $title){
			...
			if(isset($_POST['add_ct']) && $_POST['add_ct']!=""){
				...
				if($ct_item['name'] != "" ){
					...
					if(!$result = $this->link->query($sql))
						$this->db_message($sql);
          $this->adminLog("Naam {$ct_item['name']} opgevraagd van tabel $pre");
              ...
              $this->adminLog("$short {$ct_item['name']} is toegevoegd aan tabel $pre");
            }
						unset($album_item);
					}else{
    ...
    if(!$result = $this->link->query($sql))
      ...
      $this->adminLog("ID $id opgevraagd van tabel $pre");
		...
		if($result->num_rows > 0){
			$rs = $result->fetch_array(MYSQLI_BOTH);
              ...
              if(!$result = $this->link->query($sql))
                ...
                $this->adminLog("$title $id aangepast naar $new_ct van tabel $pre");
              }
					}else{
      ...
      if(!$result = $this->link->query($sql))
        ...
        $this->adminLog("ID $id opgevraagd van tabel $pre voor totaaloverzicht na aanpassing");
    ...
    if(!$result = $this->link->query($sql_c))
      ...
      $this->adminLog("ID $id opgevraagd van tabel $pre voor het verwijderen");
        ...
        $this->adminLog("ID $id verwijderd uit tabel $pre");
    ...
    $view='';
		...
		switch($view){
            ...
            $this->adminLog("IP details van ID {$_GET['edit_ip']} opgevraagd");
                  ...
                  $this->adminLog("Details van ID {$_GET['edit_ip']} geupdate");
					...
					if(isset($_POST['hostform'])){
						$rs['ip'] = trim($_POST['ip']);
						$ip = trim($_POST['ip']);
						$rs['name'] = trim(mysqli_real_escape_string($this->link, $_POST['name']));
						
						$sql="SELECT * FROM ".$prefix."known_hosts WHERE ip = '".$rs['ip']."'";
            if(!$result = $this->link->query($sql))
			      	$this->db_message($sql);
            else
              $this->adminLog("Controle op bestaan van IP {$rs['ip']}");
						
						if($result->num_rows<1){
							if($rs['ip']!="" && $this->check_valid_ip($ip)){
								$sql="INSERT INTO ".$prefix."known_hosts (ip, name)VALUES('".$rs['ip']."','".$rs['name']."')";
                if(!$result = $this->link->query($sql))
				          $this->db_message($sql);
                else{
                  $this->adminLog("IP details toegevoegd aan tabel vertrouwde gebruikers");
            ...
            if(!$result = $this->link->query($sql))
               ...
               $this->adminLog("IP details opzoeken van $delete_id");
              ...
              if(!$result = $this->link->query($sql))
                ...
                $this->adminLog("IP met ID $delete_id verwijderd");
                echo $this->message("NOTICE","Ip is verwijderd, een moment geduld...");
                echo "<meta http-equiv=\"refresh\" content=\"2;URL=index.php?section=security&view=hosts\" />";
                die();
              }
						}else{
...
<input type=\"hidden\" name=\"hostform\" value=\"add_host\" />
          ...
          if(isset($error['ip']))
					...
					echo "/></td><td><input type='text' name='name' /><input type=\"submit\" class=\"submit\" value=\"Toevoegen\" src='../images/plus.png' /></td></tr></table></form>";
                 ...
                 $this->adminLog("Status acceptatie gebruikers blog omgezet naar {$settings['website_accept_hosts']}");
					}
          ...
          if(!$result = $this->link->query($sql))
             ...
             $this->adminLog("ID {$_GET['edit_ip']} opgevraagd van blacklist");
                   ...
                   $this->adminLog("ID {$_GET['edit_ip']} van blacklist aangepast");
                  $msg['ip'] = $this->message("NOTICE",'Succesvol opgeslagen');
                }
              }else{
	...
	public function setSettings(){
		global $prefix, $settings;
			...
			if(isset($_POST['settings'])){
				...
				$settings['multiple_polls'] = intval($_POST['multiple_polls']);
				...
				if($this->isAdmin() || $this->isEditor()){
					if($settings['head_title'] != "" && $settings['max_items'] != "" && $settings['website_name'] != "" && $settings['thumb_size']!=""){
						$sql = " UPDATE ".$prefix."settings SET
									...
									multiple_poll = '".$settings['multiple_polls']."',
              ...
              $this->adminLog("Instellingen zijn aangepast");
            }
					}else{
	...
	public function authFail(){
      ...
      if(!$result = $this->link->query($sql))
			...
			//if($aantal <= 5){
				...
				$rslt = "INSERT INTO ".$prefix."log (ip, date, page, user, url, browser, webhost, os_server, os_user, referrer, protocol, method, message, extra) VALUES('".mysqli_real_escape_string($this->link, $ip)."', now(), '$page', '', '".mysqli_real_escape_string($this->link, $url)."','','$webhost','$server','$server_user','$referer','$protocol','$method','BAD_LOGIN', '$post')";			
				if(!$result = $this->link->query($rslt))
				  $this->db_message($rslt);
		//}else{
    ...
    if(!$result = $this->link->query($sql))
        ...
        if(!$result = $this->link->query($sql))
          ...
          $this->adminLog("Nieuw IP ({mysqli_real_escape_string($this->link, $ip)})  is aan blacklist toegevoegd");
			}catch (Exception $e){
    ...
    if(!$result = $this->link->query($sql))
        ...
        if(!$result = $this->link->query($sql))
           ...
           $this->adminLog("Blogitems opgehaald uit database");
              ...
              foreach ($_FILES['images']['name'] as $key => $value){		
                ...
                $i++;
				      ...
				      $this->db_message($sql);
							...
							unset($blogitem);
               $this->adminLog("Blog item toegevoegd");
		...
		public function editBlogItem($id){
			...
			if(isset($_POST['edit_blogitem'])){
                ...
                foreach ($_FILES['images']['name'] as $key => $value){								
                  ...
                  if(!$result = $this->link->query($sql))
                     ...
                     $this->adminLog("Afbeeldingen gekoppeld aan blogitem $id aangepast");
                }
							}
						}
				      ...
				      $this->db_message($sql);
							...
							unset($temp);
               $this->adminLog("Blogitem $id aangepast");
			...
			if(isset($_GET['edit_image'])){
				...
				$size=$size*2;
				$sql ="SELECT image$i, title, id, MONTHNAME(date_added) as monthname, YEAR(date_added) as year FROM blog_posts WHERE id = '$id'";
        if(!$result = $this->link->query($sql))
				  $this->db_message($sql);
 ...
 //   die($sql);
	...
	public function logOverview(){
      ...
      if(!$result = $this->link->query($sql))
          ...
          $search="";
					...
					if(isset($_POST['exclude_ip']) && $_POST['exclude_ip']!="" || isset($_GET['exlude_ip']))	
          ...
          if(!$result = $this->link->query($sql))
            ...
            if($rl['message']=="BAD_LOGIN")
              echo " style='background-color:#fcdede;' ";
             echo ">
						<td>".$rl["date"]."</td>";
    ...
    if(!$result = $this->link->query($sql))
		...
		if($result->num_rows>0){
			...
			echo "<table class='log_details'><tr class='head'><th colspan='1' class='arrow'> <a href='javascript:history.go(-1);'><img src='../images/arrow_left.png' alt='' /></a></th><th> Overzicht log entry $id</th><th>"; 
				//if($this->isEditor() || $this->isAdmin())
				...
				<tr><td class='title'>Melding</td><td>".$detailed_info["message"]."</td><td>&nbsp;</td></tr>
